-- passwdmap.hs
--
-- 기존 passwd-al에서 map을 이용한 버전으로 변형한 예제이다.
import Data.List
...
{- | The priary piece of data this program will store. It represents the
  ...
  readsPrec _ value =
    case split ':' value of
      [f1, f2, f3, f4, f5, f6, f7] ->
        -- Generate a 'PasswdEntry' the shorthand way: using the
        -- positional fields. ** We use 'read' to the numeric fields to
        -- Integers. **
    ...
    where
          ...
          split _ [] = [[]]
          split delim str =
            let -- Find the part of the list before delim and put it
                -- in "before". The reset of the list, including the
                -- leading delim, goes in "remainder".
                (before, remainder) = span (/= delim) str
                in
                before : case remainder of
                           [] -> []
                           x -> -- If there is more data to process,
                                -- call split recursively to process
                                -- it
                             split delim (tail x)
...
mainMenu maps@(uidmap, usermap) = do
  ...
  hFlush stdout
  sel <- getLine
  -- See what they want to do. For every option except 4, return them
  -- to the main menu afterwards by calling mainMenu recursively
  case sel of
